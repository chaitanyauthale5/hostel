...
const AdminReports = () => {
  ...
  const downloadReport = (data: any, reportName: string) => {
    // Create formatted Excel-like CSV content
    const csvContent = createAdvancedCSVContent(data, reportName);
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `Sunshine_Hostel_${reportName.toLowerCase().replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  const createAdvancedCSVContent = (data: any, reportName: string) => {
    ...
    // Header section with hostel branding
    csv += '=================================================\n';
    csv += 'SUNSHINE HOSTEL MANAGEMENT SYSTEM\n';
    csv += '=================================================\n';
    csv += `Report Type: ${reportName}\n`;
    csv += `Generated on: ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}\n`;
    ...
    if (Array.isArray(data.data) && data.data.length > 0) {
      ...
      csv += headers.map(h => h.toUpperCase()).join(',') + '\n';
      csv += headers.map(() => '---').join(',') + '\n'; // Separator line
      data.data.forEach((row: any) => {
        csv += headers.map(header => {
          const value = row[header] || '';
          // Format numbers with proper currency or percentage symbols
          if (typeof value === 'number' && header.includes('revenue')) {
    ...
    // Footer
    csv += '\n=================================================\n';
    csv += 'This report is confidential and generated for\n';
    csv += 'internal use only. For queries, contact admin.\n';
    csv += '=================================================\n';
